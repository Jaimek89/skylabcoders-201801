...
function jMonths() {
    var months = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december']
    var startWithJ = months.filter(function (month) {
...
function allowEntrance(age, callback) {
    age >= 18 ? callback(true) : callback(false);
}


function permission(allowed) {
    console.log(allowed ? 'You are allow to come inside' : 'Please, come back at yours 18');
}

allowEntrance(19, permission)